...
streets = gpd.read_file('Data/shp/prj_out.shp')
...
app.layout = html.Div(id="mainContainer", style={"display": "flex", "flex-direction": "column"}, children=[
    ...
    html.Div(className="row flex-display", id="header", style={"margin-bottom": "25px"}, children=[
        html.Div(className="one-third column", children=[
            html.Img(
                src=app.get_asset_url("ktc-logo.png"),
                id="plotly-image",
                style={
                    "height": "60px",
                    "width": "auto",
        ...
        html.Div(className="one-half column", id="title", children=[
            html.Div([
                html.H3('SHRP2 DASHBOARD PROTOTYPE', style={"margin-bottom": "0px"}, ),
                html.H5("PROTOTYPE OVERVIEW", style={"margin-top": "0px"})
            ])
        ]),
    ...
    html.Div(className="row flex-display", children=[
        html.Div(className="pretty_container four columns", id="cross-filter-options", children=[
            ...
            html.Label("Route Prefix: "),
            dcc.Dropdown(id="rtprefix",
                         options=[{"label": str(x), "value": x} for x in sorted(rtpref)],
                         # value="Fayette",
                         placeholder="filter routes by prefix",
                         clearable=False,
                         persistence=True,
                         persistence_type="memory",
                         className="dcc_control"),
            ...
            # html.Br(),
            # html.Button(id='trigger',
            #            n_clicks=0,
            #            children="Submit",
            #            title="click here to see graphs"),
        ]),
    ...
    #  html.Div(id="hiddenDiv", hidden=True),
    ...
    html.Div(className="row flex-display", children=[
        html.Div([dcc.Graph(id="TTic")], className="pretty_container six columns"),
        html.Div([dcc.Graph(id="TTinc")], className="pretty_container six columns")
    ]),
...
def color(x, df):
    if x < (0.6 * np.average(df['SPEED_LIMI'], weights=df['SECTION_LE'])):
        cl = 'red'
    elif x < (0.85 * np.average(df['SPEED_LIMI'], weights=df['SECTION_LE'])):
        cl = 'gold'
    else:
        cl = 'green'
    return cl
...
def updateCountyOptions(d):
    if d is not None:
        return [{"label": str(x), "value": x} for x in sorted(list(his.loc[(his['District'] == d), 'County'].unique()))]
    else:
        return [{"label": str(x), "value": x} for x in sorted(counties.keys())]


@app.callback(
    Output("rtprefix", "options"),
    [Input("district", "value"), Input("county", "value")],
    prevent_initial_call=True
)
def updatePrefix(d, c):
    if c is not None:
        p = list(his.loc[his['County'] == c, 'rtprefix'].unique())
    elif d is not None:
        p = list(his.loc[his['District'] == d, 'rtprefix'].unique())
    else:
        p = rtpref
    return [{"label": str(x), "value": x} for x in sorted(p)]


@app.callback(
    Output("route", "options"),
    [Input("district", "value"), Input("county", "value"), Input("rtprefix", "value")],
    prevent_initial_call=True
)
def updateRouteOptions(d, c, p):
    if d is not None:
        if (c is None) & (p is None):
            r = list(his.loc[(his['District'] == d), 'route'].unique())
        elif (c is not None) & (p is None):
            r = list(his.loc[(his['County'] == c), 'route'].unique())
        elif (c is None) & (p is not None):
            r = list(his.loc[(his['rtprefix'] == p) & (his['District'] == d), 'route'].unique())
        else:
            r = list(his.loc[(his['rtprefix'] == p) & (his['County'] == c), 'route'].unique())
    ...
    return [{"label": str(x), "value": x} for x in sorted(r)]
...
# updates embedded map
def updateembeddedmap(d, c):
    if c is not None:
        link = f"{embed_link}&find={c}"
    elif d is not None:
        link = f"{embed_link}&find={d}"
    else:
        link = embed_link
    return link
...
def showMPRange(r, d, c):  # optimize this later
    ...
    return "Route begins from milepoint {} to {}".format(x, y)
...
def showHS(r, b, e):
    ...
    nsum = lambda x: round(np.sum(x), 2)
    ...
    hrv = pd.DataFrame(data={'Hour': range(0,24), 'AADT': list(d.AADT.unique())*24,})
    hrv['perc'] = hrv['Hour'].map(lambda x: hfs[x])
    hrv['hvol'] = (hrv['AADT'] * hrv['perc']).round(2)
    ...
    figc.update_layout({
        'plot_bgcolor': "#F9F9F9",
        'paper_bgcolor': "#F9F9F9",
        'title': 'Average hourly speeds - Cardinal',
        'hovermode': 'x'
    })

    figc.update_xaxes(title_text="Hour of Day")

    figc.update_yaxes(title_text="Hourly speed - miles/hr", range=[min_y, max_y], secondary_y=True)
    figc.update_yaxes(title_text="Hourly volume", secondary_y=False)
    ...
    figdc.update_layout({
        'plot_bgcolor': "#F9F9F9",
        'paper_bgcolor': "#F9F9F9",
        'title': 'Average hourly Delay Cardinal',
        'hovermode': 'x'
    })

    figdc.update_yaxes(range=[min_d,max_d])
    ...
    figtdc.update_layout({
        'plot_bgcolor': "#F9F9F9",
        'paper_bgcolor': "#F9F9F9",
        'title': 'Total Delay by Year Cardinal',
        'hovermode': 'x'
    })
    ...
    fignc.update_layout({
        'plot_bgcolor': "#F9F9F9",
        'paper_bgcolor': "#F9F9F9",
        'title': 'Average hourly speeds - Non-Cardinal',
        'hovermode': 'x'
    })

    fignc.update_xaxes(title_text="Hour of Day")

    fignc.update_yaxes(title_text="Hourly speed - miles/hr", range=[min_y, max_y], secondary_y=True)
    fignc.update_yaxes(title_text="Hourly volume", secondary_y=False)
    ...
    figdnc.update_layout({
        'plot_bgcolor': "#F9F9F9",
        'paper_bgcolor': "#F9F9F9",
        'title': 'Average hourly Delay NonCardinal',
        'hovermode': 'x'
    })

    figdnc.update_yaxes(range=[min_d, max_d])
    ...
    figtdnc.update_layout({
        'plot_bgcolor': "#F9F9F9",
        'paper_bgcolor': "#F9F9F9",
        'title': 'Total Delay by Year NonCardinal',
        'hovermode': 'x'
    })

    return figc, fignc, figdc, figdnc, figtdc, figtdnc


# @app.callback(
#     [Output("TTic", "figure"), Output("TTinc", "figure")],
#     [Input("route", "value"),
#      Input("bmp", "value"),
#      Input("emp", "value")],
#     prevent_initial_call=True
# )
# def showTTiGraph(r, b, e):
#     df = his[(his['route'] == r) & (his['BEGIN_POINT'].astype(float).between(b, e, inclusive=True))]
#     c = set(df.loc[df['AllRds_Dir'] == 'Cardinal', 'LinkDir'].unique())
#     nc = set(df.loc[df['AllRds_Dir'] != 'Cardinal', 'LinkDir'].unique())
#     # cardinal
#     d = hr[hr['LinkDir'].isin(c)]
#     d = d.loc[(d.Hour >= 6) & (d.Hour < 20)].reset_index(drop=True)
#     #
#     drt, leng, adt = diction(df['LinkDir'], df['AllRds_Dir'], df['SECTION_LENGTH'], df['AADT'])
#     d['Length'] = d['LinkDir'].map(lambda x: leng[x])
#     d['HrTTi'] = d['Pcnt85Speed'] / d['Pcnt20Speed']
#     d.loc[(d.HrTTi < 0), 'HrTTi'] = 1
#     d['lwTTi'] = d['HrTTi'] * d['Length']
#     dl = d.groupby('Year', as_index=False).sum()  # change to np.average
#     dl['TTi80'] = dl['lwTTi'] / dl['Length']
#
#     figc = go.Figure(data=[
#         go.Bar(name='cardinal_TTi', x=['2018', '2019'], y=dl['TTi80'])
#     ])
#     figc.update_layout({
#         'plot_bgcolor': "#F9F9F9",
#         'paper_bgcolor': "#F9F9F9",
#         'title': 'Reliability Measure (TTi80) - Cardinal',
#         'hovermode': 'x'
#     })
#
#     # non-cardinal
#     dn = hr[hr['LinkDir'].isin(nc)]
#     dn = dn.loc[(dn.Hour >= 6) & (dn.Hour < 20)].reset_index(drop=True)
#     dn['Length'] = dn['LinkDir'].map(lambda x: leng[x])
#     dn['HrTTi'] = dn['Pcnt85Speed'] / dn['Pcnt20Speed']
#     dn.loc[(dn.HrTTi < 0), 'HrTTi'] = 1
#     dn['lwTTi'] = dn['HrTTi'] * dn['Length']
#
#     dln = dn.groupby('Year', as_index=False).sum()
#     dln['TTi80'] = dln['lwTTi'] / dln['Length']
#
#     fignc = go.Figure(data=[
#         go.Bar(name='non-cardinal', x=['2018', '2019'], y=dln['TTi80'])
#     ])
#     fignc.update_layout({
#         'plot_bgcolor': "#F9F9F9",
#         'paper_bgcolor': "#F9F9F9",
#         'title': 'Reliability Measure (TTi80) - NonCardinal',
#         'hovermode': 'x'
#     })
#
#     return #figc, fignc
...
def showMapboxplot(r, b, e):
    ...
    fig = go.Figure(
        data=d['data'],
        layout=d['layout'])
    return dcc.Graph(id="mapbox_plot", figure=fig)
