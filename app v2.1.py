...
# APP LAYOUT
app.layout = html.Div(id="mainContainer",
                      style={"display": "flex", "flex-direction": "column", "max-width": "2200px", "margin": "auto"},
                      children=[
                          html.Div(id="output-clientside"),  # empty Div to trigger javascript file for resizing
                          html.Div(className="row flex-display", id="header", style={"margin-bottom": "25px"},
                                   children=[
                          ...
                          html.Div(className="row flex-display", children=[
                              html.Div(className="pretty_container four columns", id="cross-filter-options", children=[
                                  html.P("Select filters below:", className="control_label",
                                  ...
                                  html.Label("County: "),
                                  html.Label(' Required for non-interstate and non-parkway routes', className="control_label"),
                          ...
                          html.Div([
                              dcc.ConfirmDialog(id='confirm',
                                                message="Selected route not an Interstate or Parkway. Select a County")
                          ]),
                          ...
                          html.Div(id="graphdiv", children=[
                              html.Div(id='tabs_div', children=[dcc.Tabs(id='graph-tabs',
                                                                         ...
                                                                         children=[
                                                                             dcc.Tab(label='2018 - 2019 Statistics',
                                                                                     value='tab-1', style=tab_style,
                                                                                     selected_style=tab_selected_style),
                                                                             dcc.Tab(label='2015 - 2019 Statistics',
                                                                                     value='tab-2', style=tab_style,
                                                                                     selected_style=tab_selected_style),
                                                                         ]),
                                                                ]),
                              html.Br(),
...
# countinuous routes = crt, llst of crt's below
crt = ['AU', 'BG', 'EB', 'HR', 'I ', 'JC', 'LN', 'WK', 'WN']
...
def showMPRange(r, d, c):  # optimize this later
    if r is None:
        ...
        elif (d is not None) and (r.split('-')[0] in crt):
...
@app.callback(
    [Output("bmp", "min"), Output("bmp", "max"), Output("emp", "min"), Output("emp", "max"), Output("bmp", "value"),
     Output("emp", "value"), Output('confirm', 'displayed')],
    [Input("route", "value"), Input("district", "value"), Input("county", "value")],
    prevent_initial_call=True
)
def define_mpinput_min_max(r, d, c):  # optimize this later. create a hidden div to share data between callbacks
    if c is not None:
        ...
        return bmn, bmx, emn, emx, bmn, emx, False
